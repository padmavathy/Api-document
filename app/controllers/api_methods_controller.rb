class ApiMethodsController < ApplicationController
  ...
  def index
    if params[:id]
      @api_methods = ApiMethod.list_methods(params[:id])
    else
  ...
  def method_desc
    ...
    @api_list = ApiMethod.find(params[:id])
    @api_list.destroy
    respond_to do |format|
      format.html { redirect_to(api_methods_url) }
      format.xml  { head :ok }
  ...
  def post_api_request
    ...
    response=HTTParty.post(params[:url],{:body=>{:access_token=>access_token}.merge(hash)})
 ...
 def put_api_request
    hash = ActiveSupport::JSON.decode(params['body'])
    response = HTTParty.put(params[:url],{:body=>{:access_token=>access_token}.merge(hash)})
    result =   JSON.parse(response.body)
    res = JSON.pretty_generate(result)
  ...
  def get_api_request
    hash = ActiveSupport::JSON.decode(params['body'])
    response=HTTParty.get(params[:url],:query=>{:access_token=>access_token})
    result =   JSON.parse(response.body)
    res = JSON.pretty_generate(result)
    if response['response'] == "success"
      render :json=>{:response=>res, :status => 'success'}
  ...
  def delete_api_request
    hash = ActiveSupport::JSON.decode(params['body'])
    response=HTTParty.delete(params[:url],:query=>{:access_token=>access_token}).parsed_response
